...
class Settings:
    susscan_home: Path = Path(os.getenv("SUSSCAN_HOME", "/opt/SusScan"))
    app_dir: Path = Path(os.getenv("SUSSCAN_APP_DIR", "/opt/SusScan/app"))
    data_dir: Path = Path(os.getenv("SUSSCAN_DATA_DIR", "/opt/SusScan/data"))
    uploads_dir: Path = Path(os.getenv("SUSSCAN_UPLOADS_DIR", "/opt/SusScan/data/uploads"))
    jobs_dir: Path = Path(os.getenv("SUSSCAN_JOBS_DIR", "/opt/SusScan/data/jobs"))
    reports_dir: Path = Path(os.getenv("SUSSCAN_REPORTS_DIR", "/opt/SusScan/data/reports"))
    decompiled_dir: Path = Path(os.getenv("SUSSCAN_DECOMPILED_DIR", "/opt/SusScan/data/decompiled"))
    rules_dir: Path = Path(os.getenv("SUSSCAN_RULES_DIR", "/opt/SusScan/rules"))
    db_path: Path = Path(os.getenv("SUSSCAN_DB_PATH", "/opt/SusScan/data/SusScan.db"))
    compiled_yara_path: Path = Path(
        os.getenv("SUSSCAN_COMPILED_YARA_PATH", "/opt/SusScan/rules/compiled/all_combined.yarc")
    )

    max_upload_size_mb: int = _env_int("SUSSCAN_MAX_UPLOAD_SIZE_MB", 100)
    ...
    reputation_timeout_seconds: int = _env_int("SUSSCAN_REPUTATION_TIMEOUT_SECONDS", 10)
    min_malicious_provider_hits_for_known_malicious: int = _env_int(
        "SUSSCAN_MIN_MALICIOUS_PROVIDER_HITS_FOR_KNOWN_MALICIOUS",
        2,
    )
    min_positive_provider_hits_for_suspicious: int = _env_int(
        "SUSSCAN_MIN_POSITIVE_PROVIDER_HITS_FOR_SUSPICIOUS",
        1,
    )
    ...
    groq_timeout_seconds: int = _env_int("SUSSCAN_GROQ_TIMEOUT_SECONDS", 45)
    groq_max_completion_tokens: int = _env_int("SUSSCAN_GROQ_MAX_COMPLETION_TOKENS", 700)
    groq_temperature: float = _env_float("SUSSCAN_GROQ_TEMPERATURE", 0.2)
    report_chat_history_turn_limit: int = _env_int("SUSSCAN_REPORT_CHAT_HISTORY_TURN_LIMIT", 8)
